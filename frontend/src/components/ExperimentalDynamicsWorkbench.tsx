...
export function ExperimentalDynamicsWorkbench({ corpusId }: { corpusId: string }) {
  ...
  return <main className="experimental-workbench" aria-label="Экспериментальные модели">
    ...
    <section className="experimental-grid">
      <article><h2>Semantic transfer entropy</h2><small>{data.semantic_information_dynamics.status} · {data.semantic_information_dynamics.representation ?? data.semantic_information_dynamics.reason}</small>
      ...
      <article><h2>Лингвистический affect</h2><small>{data.linguistic_affect_dynamics.interpretation}</small><p>{data.linguistic_affect_dynamics.messages_with_nonzero_signal} / {data.linguistic_affect_dynamics.messages} сообщений с ненулевым сигналом</p>
      ...
      <article className="experimental-wide"><h2>Partial information decomposition</h2>
